...
Description: This script contains the main functions required to train YOLOv5.
...
def train(hyp, opt, device, callbacks):  # hyp is path/to/hyp.yaml or hyp dictionary
    ...
    #Initialise epoch and classAPs headers and results (epoch header obsolete)
    #epochHeader = ["Epoch", "Time (s)"]
    classAPsHeader = ["Epoch", "Class 0", "Class 1", "Class 2"]
    ...
    for epoch in range(start_epoch, epochs): 
        ...
        if RANK in {-1, 0}:
            ...
            if not noval or final_epoch:  # Calculate mAP
                results, maps, _, classAPs = validate.run(data_dict,
        ...
        epochTimes.append([epoch, epoch_time])
        
        classAPs.insert(0, epoch)
        classAPsResults.append(classAPs)
    
        # # Save epoch times
        # with open('epochTimes.csv', 'w', newline='') as file:
        #     writer = csv.writer(file)
            
        #     # Write the header row
        #     writer.writerow(epochHeader)
            
        #     # Write each sublist as a row in the CSV file
        #     for sublist in epochTimes:
        #         writer.writerow(sublist)
                
        print("CLASS APS: ", classAPs)

        print(f"Epoch {epoch+1} training time: {epoch_time:.2f} seconds")
               
    # Save classAPs
    with open('classAPsResults.csv', 'w', newline='') as file:
        writer = csv.writer(file)
        writer.writerow(classAPsHeader)
        
        for sublist in classAPsResults:
            writer.writerow(sublist)
    ...
    if RANK in {-1, 0}:
        ...
        overallTrainingTime = (time.time() - t0) 
        epochTime = ['Overall', overallTrainingTime]
        
        #Save epoch times
        with open('epochTimes.csv', 'a', newline='') as file:
            writer = csv.writer(file)
        ...
        for f in last, best:
            if f.exists():
                ...
                if f is best:
                    LOGGER.info(f'\nValidating {f}...')
                    results, _, _, classAPs = validate.run(
                        data_dict,
                        batch_size=batch_size // WORLD_SIZE * 2,
                        imgsz=imgsz,
    ...
    print("classAP Results: ", classAPsResults)
    return results
